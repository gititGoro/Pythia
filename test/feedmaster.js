...
contract('FeedMaster', function (accounts) {
    ...
    describe("push new feed, get ID, use ID to get details", () => {
        ...
        test("should push new feed", async () => {
            await feedMasterInstance.pushNewFeed(4, 5, 100, "ETHZAR", "ether rand exchange rate", { from: accounts[0], value: 400 });
            ...
            assert.equal(detail[2], "100", "max range");
            assert.equal(detail[3], "4", "decimal places");
            assert.equal(detail[4], "5", "number of oracles");
            assert.equal(detail[5], "ether rand exchange rate", "description");
        });

        test("should fail when not sending ether", async () => {
            await expectThrow(feedMasterInstance.pushNewFeed(4, 5, 100, "ETHZAR", "ether rand exchange rate", { from: accounts[0] }));
        });
    });
    ...
    describe("push multiplefeeds, get back multiple IDS", () => {
        ...
        test("push multiple feeds of same type", async () => {
            ...
            await feedMasterInstance.pushNewFeed(4, 5, 20, "ETHZAR", "ether rand exchange rate", { from: accounts[0], value: 400 });
            idArray = await feedMasterInstance.getIDsForFeed.call("ETHZAR");
            assert.equal(idArray.length, 3, "array length 3");
            await feedMasterInstance.pushNewFeed(4, 5, 30, "ETHZAR", "ether rand exchange rate", { from: accounts[0], value: 400 });
            idArray = await feedMasterInstance.getIDsForFeed.call("ETHZAR");
        ...
        test("push interleaved feeds of different types to test id assignment", async () => {
            await feedMasterInstance.pushNewFeed(14, 5, 100, "USDGBP", "dollar pound", { from: accounts[0], value: 4 });
            await feedMasterInstance.pushNewFeed(14, 5, 100, "JPYSWC", "yen swiss exchange rate", { from: accounts[0], value: 4 });
            await feedMasterInstance.pushNewFeed(14, 5, 100, "USDGBP", "back to pound", { from: accounts[0], value: 4 });
            await feedMasterInstance.pushNewFeed(14, 5, 100, "JPYSWC", "back to yen", { from: accounts[0], value: 4 });
            await feedMasterInstance.pushNewFeed(14, 5, 100, "ETHZAR", "back to ether zar", { from: accounts[0], value: 4 });
            ...
            assert.equal(detail[2], "100", "max range");
            assert.equal(detail[3], "14", "decimal places");
            assert.equal(detail[4], "5", "number of oracles");
            assert.equal(detail[5], "yen swiss exchange rate", "description");
        });
